...
def remove_stopwords(file):
    ...
    file['text'] = file['text'].apply(lambda x: " ".join(
        x for x in x.split() if x not in stop and not x.isdigit()))
    return file
...
def lemmatization(file):
    ...
    file['text'] = file['text'].apply(lambda x: " ".join(
        [lemmer.lemmatize(word) for word in x.split()]))
    return file
